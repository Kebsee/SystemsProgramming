 ...
 **		adds globbing support to smsh3
 **		supports commands like: ls *.c, cat *.h
 **/
 ...
 char **expand_globs(char **arglist);
 int has_wildcards(char *str);
 
 int main()
 {
     char	*cmdline, *prompt, **arglist, **expanded_arglist;
     int	result;
     void	setup();
     ...
     while ( (cmdline = next_cmd(prompt, stdin)) != NULL ){
         if ( (arglist = splitline(cmdline)) != NULL  ){
             /* Expand wildcards */
             expanded_arglist = expand_globs(arglist);
             freelist(arglist);
             
             if (expanded_arglist != NULL) {
                 if ( find_pipe(expanded_arglist) != -1 )
                     result = execute_pipe(expanded_arglist);
                 else if ( find_redirection(expanded_arglist, ">") != -1 || find_redirection(expanded_arglist, "<") != -1 )
                     result = execute_with_redirection(expanded_arglist);
                 else
                     result = execute_single(expanded_arglist);
                 freelist(expanded_arglist);
             }
         }
         free(cmdline);
     }
 ...
 int has_wildcards(char *str)
 /*
  * purpose: check if string contains wildcard characters
  * returns: 1 if wildcards found, 0 otherwise
  */
 {
 ...
 char **expand_globs(char **arglist)
 /*
  * purpose: expand wildcard patterns in argument list
  * returns: new argument list with expanded patterns
  */
 {
     char **result;
     int result_size = 0;
     ...
     result = emalloc(result_capacity * sizeof(char *));
     
     while (arglist[i] != NULL) {
         if (has_wildcards(arglist[i])) {
             glob_t glob_result;
             int glob_flags = 0;
             
             if (glob(arglist[i], glob_flags, NULL, &glob_result) == 0) {
                 /* Add all matched files */
                 for (size_t j = 0; j < glob_result.gl_pathc; j++) {
                     /* Ensure we have space */
                     if (result_size >= result_capacity - 1) {
                         result_capacity *= 2;
                         result = erealloc(result, result_capacity * sizeof(char *));
                     }
                     result[result_size++] = strdup(glob_result.gl_pathv[j]);
                 }
                 ...
                 result[result_size++] = strdup(arglist[i]);
             }
         } else {
